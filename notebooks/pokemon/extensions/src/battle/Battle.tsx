...
import { WidgetPluginProps, parse, useDataModel, getPage, createFilter } from "@activeviam/activeui-sdk";
...
const POKEMON_LEVEL = "Pokemon";
const OPPONENT_LEVEL = "Opponent Pokemon";
...
export const Battle: FC<WidgetPluginProps<BattleWidgetState>> = (props: any) => {
  ...
  const selectPokemonRef = useRef<HTMLSelectElement>(null);
  const [displaySelectPokemon, setDisplaySelectPokemon] = useState(false);
  const selectOpponentRef = useRef<HTMLSelectElement>(null);
  ...
  useEffect(() => {
    ...
    const updatedDashboardState = produce(props.dashboardState, (draftDashboardState) => {
      ...
      pageState.filters = [
        ...removeFilters(pageState.filters, [POKEMON_LEVEL, OPPONENT_LEVEL]),
      ];
      ...
      Object.values(pageState.content).forEach(widget => {
        if (widget.widgetKey === props.widgetState.widgetKey) {
          ...
          widget.filters = [
            ...removeFilters(widget.filters, [POKEMON_LEVEL, OPPONENT_LEVEL]),
          ];
        }
      });
    });
  ...
  const generateSelectOptions = (pokemonList) => {
  ...
  const removeFilters = (filters, hierarchyList) => {
    return (filters || []).filter((mdx) => {
      const filter = createFilter(mdx, cube);
      return !hierarchyList.includes(filter.hierarchyName);
    })
  }

  const createFilters = (obj) => {
    return Object.entries(obj).map(([key, value]) => createMdxFilter(key, value));
  }

  const createMdxFilter = (hierarchy, member) => {
    return {
      ...parse(`[${hierarchy}].[${hierarchy}].[AllMember].[${member}]`),
      type: "member",
      dimensionName: hierarchy,
      hierarchyName: hierarchy,
      levelName: hierarchy,
      path: ["AllMember", member],
    }
  }
        ...
        { displaySelectPokemon 
          ...
          : <div>
              <img 
                ...
                src={ require('../resources/images/crown.png').default }
              />
              <Card 
