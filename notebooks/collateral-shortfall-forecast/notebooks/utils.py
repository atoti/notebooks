...
from tsfresh.utilities.dataframe_functions import impute, make_forecasting_frame, roll_time_series, restrict_input_to_index
...
def create_forecasting_frame(dataframe, col, max_timeshift, rolling_direction):
    ...
    date_df, _ = make_forecasting_frame(dataframe['Date'], kind='Date', max_timeshift=max_timeshift, rolling_direction=rolling_direction)
    value_df, y = make_forecasting_frame(dataframe[col], kind=col, max_timeshift=max_timeshift, rolling_direction=rolling_direction)
    
    X = pd.merge(value_df, date_df, how='inner', on=['id', 'time'])
    X = X.rename(columns={'value_x': 'asset_value', 'value_y': 'date', 'kind_x': 'asset_code'})
...
def update_test_data(test_data_path):
    ...
    max_timeshift=10
    rolling_direction=1
    col = test_data.columns[1] # we choose the columns corresponding to the first asset arbitrarily 
    
    date_df, _ = make_forecasting_frame(test_data['Date'], kind='Date', max_timeshift=max_timeshift, rolling_direction=rolling_direction)
    value_df, y = make_forecasting_frame(test_data[col], kind=col, max_timeshift=max_timeshift, rolling_direction=rolling_direction)
...
def create_and_save_dataset(forecasting_frame_tuple, path, horizon, split):
    ...
    filename = os.path.join(path, 'rolled-dataset-' + horizon + '-' + dataset_name + '-' + split + '.pkl')
...
def build_models_and_results_summary(results_path, eval_metric='RMSE'):
    ...
    assert eval_metric in ['R2', 'RMSE', 'MAPE'], "The evaluation metric must be 'R2', 'RMSE', or 'MAPE'."
    ...
    with tqdm(total=values, file=sys.stdout) as pbar:
        for horizon in horizons:
            for asset in assets:
                train = pd.read_csv(os.path.join(results_path, 'features', 'features-' + horizon + asset + '-train.csv'), index_col=0)
                test = pd.read_csv(os.path.join(results_path, 'features', 'features-' + horizon + asset + '-test.csv'), index_col=0)
                ...
                results_dic = {'R2': {'PLS': r2_pls, 'O-PLS': r2_opls, 'RF': r2_rf, 'GBoost': r2_gb},
                               'RMSE': {'PLS': rmse_pls, 'O-PLS': rmse_opls, 'RF': rmse_rf, 'GBoost': rmse_gb},
                               'MAPE': {'PLS': mape_pls, 'O-PLS': mape_opls, 'RF': mape_rf, 'GBoost': mape_gb}}
                
                models_perf = [(model, perf) for model, perf in results_dic[eval_metric].items()] 
                ...
                pickle_file = os.path.join(results_path, 'models',  model_name + '-' + horizon + '-' + asset + '.pkl')
                with open(pickle_file, "wb") as fOut:
                    pickle.dump({'model': best_model}, fOut, protocol=pickle.HIGHEST_PROTOCOL)
                ...
                results['forecasting horizon in days'] = [horizon.split('-')[0]] * len(y_test)
                ...
                # save the tables
                results.to_csv(os.path.join(results_path, 'predictions', model_name + '-predictions-' + horizon + asset + '-test.csv'))
                summary_table.to_csv(os.path.join(results_path, 'summary', model_name + '-summary-' + horizon + asset + '-test.csv'))
