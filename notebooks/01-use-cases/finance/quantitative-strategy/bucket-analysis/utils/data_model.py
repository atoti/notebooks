...
def enrich_cube(session):
    ...
    sector_tbl = session.read_csv(
        "s3://data.atoti.io/notebooks/bucket-exploration/constituents.csv", table_name="Sector", keys=["Symbols"]
    )
    ...
    txn_tbl.join(hist_tbl, (txn_tbl["Ticker"] == hist_tbl["Ticker"]) & (txn_tbl["DateTime"] == hist_tbl["DateTime"]))
