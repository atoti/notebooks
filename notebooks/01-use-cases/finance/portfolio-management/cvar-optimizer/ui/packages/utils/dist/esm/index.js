import { getMeasure, createFilter, getDimension, getHierarchy, getLevel } from '@activeviam/activeui-sdk';
import { getLevelCoordinates, getChildrenLevels } from '@activeviam/data-model';
import { isMdxFunction, getSpecificCompoundIdentifier, getHierarchies } from '@activeviam/mdx';
...
function createExplicitMemberPayload(dimensionName, hierarchyName, members, slicing, cube) {
  ...
  const firstLevel = getLevel(levelCoordinates, cube);
  const childrenLevels = getChildrenLevels(levelCoordinates, cube);
...
function createMdxLocationPayload(mdx, cube) {
  if (isMdxFunction(mdx, "filter")) {
    ...
    if (isMdxFunction(membersNode, "members")) {
      ...
      const specificCompoundIdentifier = getSpecificCompoundIdentifier(compoundIdentifier, {
        cube
      });
  ...
  if (isMdxFunction(mdx) && ["topcount", "topsum", "toppercent", "bottomcount", "bottomsum", "bottompercent"].includes(mdx.name.toLowerCase())) {
    const firstArgumentOfFilter = mdx.arguments[0].arguments[0];
    ...
    const [{
      dimensionName,
      hierarchyName
    }] = getHierarchies(hierarchyCompoundIdentifier, {
      cube
    });
...
function transformMemberToMeasure(member, cube) {
  const measures = member.tuple.filter(value => value.dimensionName == "Measures").map(value => getMeasure(value.captionPath[0], cube));
...
function parsePayloadToFilter(locationPayload, cube) {
  return isExplicitMemberPayload(locationPayload) ? {
    ...
    type: "members"
  } : createFilter(locationPayload.mdx, cube);
}
...
export { createSecureRestQuerierFrom, getFiltersFromLocations, isDimensionWithinLocationInfo, isHierarchyWithinLocationInfo, isLevelWithinLocationInfo, isOnAxisOnly, parseSelectionToLocationInfo, retrieveMembersOfLevelFromLocationInfo, useSecureRestQuerier, useSecurityRequestOptions };
