...
class MakePredictions():
    ...
    def get_detections_from_camera(self, masked_face_detector, conf_level, video_from_camera, output_folder):
        """
        ...
        cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = self.conf_level # 0.5 set threshold for this model
...
#                 cv2.imshow('frame', out.get_image()[:, :, ::-1])
                ...
                if not labels:
                    ...
                    qtr_ts = self.ceil_dt(now, timedelta(seconds=time_increment))
                    quarter_time_stamp += [qtr_ts.strftime("%Y-%m-%d %H:%M:%S")]* len(current_labels)
                ...
                if current_time - prev_time >= time_increment:
                    ...
                    prediction_df['face_id']=prediction_df.groupby("timestamp").cumcount()
                    prediction_df = prediction_df[['face_id', 'predictions', 'label', 'timestamp', 'quarter_timestamp', 'video_id']]
...
#                     print(os.path.join(output_folder, output_csv))
                    ...
                    with open(os.path.join(output_folder, output_csv), 'a') as f:
                        ...
                        prediction_df.to_csv(f, header=f.tell()==0, index=False,line_terminator='\n')
